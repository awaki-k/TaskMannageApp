...
export default function App() {
    ...
    const handleGenerateReport = async (userResponses: Record<string, { blocker: string; progress: string }>) => {
        ...
        const systemInstruction = `あなたは優秀なプロジェクトアシスタントです。上司に報告するための日報を作成してください。
以下の進捗情報とユーザーの自己申告を基に、簡潔な日報をマークダウン形式で作成してください。
ユーザーからの入力がないタスクについても、客観的な事実（遅延している、期限が近いなど）に基づいて、上司に報告するための文章を作成して。

出力形式:
# ${dateString}の日報

## 今日の成果
（完了タスクを基に、要約を生成、上司に報告するための日本語で。）

## 現在の課題と次のアクション
（遅延タスクとユーザーの回答を基に、課題点を整理し、具体的な次のアクションを提示。上司に報告するための日本語で。）

## 今週のフォーカス
（期限が迫っているタスクに言及し、注意を促す。上司に報告するための日本語で。）
`;
        // ▲▲▲ 修正箇所 ▲▲▲
        ...
        try {
            ...
            setGeneratedReport("エラー：日報の生成に失敗しました。APIキーが正しく設定されているか確認するか、時間をおいて再度お試しください。");
        } finally {
            setIsGenerating(false);
        }
    };
